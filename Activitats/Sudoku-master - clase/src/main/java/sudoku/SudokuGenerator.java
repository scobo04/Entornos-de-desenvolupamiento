    ...
    private boolean backtrackSudokuSolver(int r,int c,SudokuPuzzle puzzle) {
		...
		if(puzzle.isSlotAvailable(r, c)) {
			...
			for(int i = 0;i < puzzle.getValidValues().length;i++) {
				...
				boolean isInRow = puzzle.numInRow(r, puzzle.getValidValues()[i]);
				boolean isInCol = puzzle.numInCol(c,puzzle.getValidValues()[i]);
				boolean isInBox = puzzle.numInBox(r,c,puzzle.getValidValues()[i]);
